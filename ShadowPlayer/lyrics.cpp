...
//歌词解析，传入参数：音频文件名
bool Lyrics::resolve(QString fileName, bool isLrc)
{
...
bool Lyrics::loadFromLrcDir(QString fileName)
{
    QFileInfo fileInfo(fileName);
    fileName = QCoreApplication::applicationDirPath() + "/lyrics/" + fileInfo.completeBaseName() + ".lrc";
...
bool Lyrics::loadFromFileRelativePath(QString fileName, QString path)
{
    QFileInfo fileInfo(fileName);
    fileName = fileInfo.path() + path + fileInfo.completeBaseName() + ".lrc";
