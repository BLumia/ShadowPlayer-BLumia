...
static HSTREAM nowPlayForJump;
static double jumpToTime;
void CALLBACK callPlayerJump(HSYNC handle, DWORD channel, DWORD data, void *user)
{
    BASS_ChannelSetPosition(nowPlayForJump, BASS_ChannelSeconds2Bytes(nowPlayForJump, jumpToTime), BASS_POS_BYTE);
}
...
QString Player::openFile(QString fileName)
{
            ...
            )
    {
        ...
        nowPlay = BASS_StreamCreateFile(false,fileName.toStdWString().c_str(),0,0,BASS_UNICODE|BASS_SAMPLE_FLOAT|BASS_SAMPLE_FX|BASS_STREAM_DECODE|BASS_STREAM_PRESCAN);
...
void Player::setPosByTime(double timeSec)
{
    BASS_ChannelSetPosition(nowPlay, BASS_ChannelSeconds2Bytes(nowPlay, timeSec), BASS_POS_BYTE);
}
...
{
    QString mono = QString::fromUtf8("单声道");
    QString stereo = QString::fromUtf8("立体声");
    QString fmt = QString("");
    BASS_CHANNELINFO cInfo;
    ...
    if (info->chans == 1)
        return QString::number(info->freq) + "Hz " + QString::number(this->getBitRate()) + "Kbps " + mono + fmt;
    else
        return QString::number(info->freq) + "Hz " + QString::number(this->getBitRate()) + "Kbps " + stereo + fmt;
}
...
DWORD Player::getLevel()
{
    ...
    DWORD level;
    level = BASS_ChannelGetLevel(nowPlay);
    if (level != -1) {
        return level;
    }
    else
...
void Player::setJumpPoint(double timeFrom, double timeTo)
{
    BASS_ChannelRemoveSync(nowPlay, jumping);
    jumping = BASS_ChannelSetSync(nowPlay, BASS_SYNC_POS, BASS_ChannelSeconds2Bytes(nowPlay, timeFrom), callPlayerJump, 0);
    jumpToTime = timeTo;
    nowPlayForJump = nowPlay;
}

void Player::removeJumpPoint()
{
    BASS_ChannelRemoveSync(nowPlay, jumping);
    jumping = 0;
    jumpToTime = 0;
